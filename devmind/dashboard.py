...
from devmind.config import load_config, save_config, get_blocklist, add_block, remove_block

CONFIG_PATH = os.path.join(os.path.dirname(__file__), "../config.json")
LOG_PATH = os.path.expanduser("~/.devmind/focus_log.json")
NOTES_PATH = os.path.expanduser("~/.devmind/session_notes.txt")
...
def run_session():
    console.clear()
    config = load_config()
    rank = get_rank_title(config.get('level', 1))
    
    console.print(Panel(f"[bold white]INITIALIZING DEVMIND OS v8.2[/]\n[dim]OPERATOR: {rank} (LVL {config.get('level', 1)})[/]", border_style="white"))
    ...
    with Live(layout, refresh_per_second=10, screen=True) as live:
        while not should_exit:
                ...
                if char == '\r':
                    ...
                    if base in ["stop", "exit"]: should_exit=True; save_session=True
                        ...
                        if args: topic = " ".join(args); events.append(f"[white]>> NEW TOPIC: {topic}[/]")
                         ...
                         if args:
                             add_block(args[0])
                             events.append(f"[white]>> ADDED BLOCK: {args[0]}[/]")
                    elif base == "del":
                         if args:
                             remove_block(args[0])
                             events.append(f"[white]>> REMOVED BLOCK: {args[0]}[/]")
                    elif base == "list": 
                         blocked = get_blocklist()
                         events.append(f"[dim]BLOCKS: {', '.join(blocked[:5])}...[/]")
                    ...
                    elif base == "rank": events.append(f"[white]>> RANK: {rank} (LVL {config.get('level', 1)})[/]")
                    elif base == "xp": events.append(f"[white]>> XP: {config.get('xp', 0)}[/]")
                    elif base == "note":
                        if args:
                             try:
            ...
            border_col = "white"
            if not paused:
                # Direct ASCII Interception overlay trigger!
                if blocked_site := scan_and_close_windows(get_blocklist(), sound_enabled):
    ...
    if save_session:
        ...
        config['xp'] = config.get('xp', 0) + xp_gain
        config['total_focus_minutes'] = config.get('total_focus_minutes', 0.0) + final_mins
        leveled, xp_req = check_level_up(config['xp'], config.get('level', 1))
        if leveled: 
            config['level'] = config.get('level', 1) + 1
            config['xp'] -= xp_req
...
def get_main_menu_panel():
    config = load_config()
    rank = get_rank_title(config.get('level', 1))
    xp_req = int(100 * (config.get('level', 1) ** 1.5))
    ...
    stat_block = f"\n[bold white]{rank}[/] (LVL {config.get('level', 1)})\n[dim]XP: {config.get('xp', 0)} / {xp_req}[/]"
...
def manage_list(config, list_key, prompt):
    while True:
        ...
        for i, v in enumerate(items): table.add_row(str(i+1), v)
        console.print(table)
